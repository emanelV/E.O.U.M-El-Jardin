...
for($i = 0; $i < count($consu); $i++){
    ...
    $consu[$i]['acciones'] = '
    <button class="btn btn-primary" title="Editar" onclick="editarAula('.$consu[$i]['aula_id'].')">Editar</button> 
    <button class="btn btn-danger" title="Eliminar" onclick="eliminarAula('.$consu[$i]['aula_id'].')">Eliminar</button>
    ';
}
